...
import edu_up_cs301.game.actionMsg.GameOverAckAction;
import edu_up_cs301.game.actionMsg.MyNameIsAction;
import edu_up_cs301.game.actionMsg.ReadyAction;
import edu_up_cs301.game.infoMsg.BindGameInfo;
import edu_up_cs301.game.infoMsg.GameInfo;
import edu_up_cs301.game.infoMsg.GameOverInfo;
import edu_up_cs301.game.infoMsg.StartGameInfo;
import edu_up_cs301.game.infoMsg.TimerInfo;
import edu_up_cs301.game.util.GameTimer;
import edu_up_cs301.game.util.MessageBox;
import edu_up_cs301.game.util.Tickable;
...
public abstract class GameHumanPlayer implements GamePlayer, Tickable {
	...
	private class Unflasher implements Runnable {
		...
		public void run() {
			View top = edu_up_cs301.game.GameHumanPlayer.this.getTopView();
			if (top == null) return;
		...
		public void run() {
			...
			if (game == null) {
				...
				if (myInfo instanceof BindGameInfo) {
					...
					game.sendAction(new MyNameIsAction(edu_up_cs301.game.GameHumanPlayer.this, name));
				}
			}
				...
				if (myInfo instanceof StartGameInfo) {
					...
					game.sendAction(new ReadyAction(edu_up_cs301.game.GameHumanPlayer.this));
				}
			}
				...
				game.sendAction(new GameOverAckAction(edu_up_cs301.game.GameHumanPlayer.this));
